...
export interface AppliedSekerItem{
  tracking_id: number;
  company_id: number;
  post_id: number;
  user_id: number;
...
interface TrackingProps {
  initialJobs: AppliedSekerItem[];
  companyId:string;
}

export default function SeekerApplication({ initialJobs }: TrackingProps) {
  console.log("initialJobสสสสสสสสสส",initialJobs);
  const [jobs] = useState<AppliedSekerItem[]>(initialJobs);

  const [selectedJob, setSelectedJob] = useState<AppliedSekerItem | null>(
